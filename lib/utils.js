...
export async function generatePDF(reportText, patientName) {
  ...
  doc.text("CareContact — Pre-Consultation Report", pageW / 2, 12, { align: "center" });
  doc.setFontSize(9);
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.text(`CareContact · Page ${i} of ${total} · ${patientName || "Patient"}`, pageW/2, pageH-8, { align:"center" });
  }
  return doc;
}
